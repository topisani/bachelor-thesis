...
$\Resample\left<N\right>(d)$ block is introduced, which can be used to perform over/undersampling for the block
$d$.

\begin{prooftree}
  \AxiomC{$d : \Sig^{n} \rightarrow \Sig^m$}
  \UnaryInfC{$\Resample\langle N\rangle(d) : \Sig^n \rightarrow \Sig^m$}
\end{prooftree}
